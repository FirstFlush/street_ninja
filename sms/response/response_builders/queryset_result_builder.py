...
#     def __init__(self, resources: ResourceQuerySet, offset: int, params: dict[str, Any ] | None = None):
#         self.resources = resources
#         self.offset = offset
#         self.params = params
#         if isinstance(self.resources, ResourceQuerySet) and len(self.resources) > 0:
#             self.keyword_enum = self.resources.first().keyword_enum
#         else:
#             msg = f"{self.__class__.__name__} received invalid resources upon instantiation. Type: `{self.resources.__class__.__name__}`, resources: `{self.resources}`"
#             logger.error(msg)
#             raise TypeError(msg)
...
#     def _create_queryset_response(self, resources:ResourceQuerySet) -> str:
#         response_items = []
#         for instance in resources:
#             res = self.template.format_result(instance=instance)
#             response_items.append(res)
